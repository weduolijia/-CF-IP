...
After that, only rows that also return a latency from the configured TCPing API are kept. The default `single`
mode uses `CN_TCPING_API` only; the optional `two` mode requires an explicit `LATENCY_API_B` and is intended
for users who want a second independent latency source.
